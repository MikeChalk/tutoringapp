...
function getNextPayout(frequency: string, lastPayout: Date | null): string | null {
  ...
  if (frequency === "monthly") {
    const next = new Date(lastPayout)
...
export default async function PayrollPage(props: { searchParams: Promise<{ year?: string; from?: string; to?: string }> }) {
  await requireAdmin()

  const { year: yearParam, from: fromParam, to: toParam } = await props.searchParams
  ...
  const where: Record<string, unknown> = { category: "TUTOR_PAY" }
  if (dateRange) {
  ...
  return (
    <div>
      ...
      {expenses.length === 0 && dateRange ? (
        <EmptyState icon={DollarSign} title="No data for this period" description="No payroll entries found in the selected time range." />
      ) : (
